...
@Component({
  selector: 'app-recette-equipe',
  templateUrl: './recette-equipe.component.html',
  styleUrls: ['./recette-equipe.component.css']
})
export class RecetteEquipeComponent implements OnInit {
    ...
    if (this.Livrables == []) {
      ...
      if (this.SelectedE) {
        if (this.DateR) {
          ...
          r.forEach((item) => {
            this.Enveloppe = this.Enveloppe + item.recettej;
          });
        }
      } else {
        this.Enveloppe = 0;
